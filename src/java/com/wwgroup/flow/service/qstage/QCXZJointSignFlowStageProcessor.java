...
/**
 * 行政职能部门会办
 * 这里会办要注意，在找对应的单位主管时找部门主管，然后再由部门主管去呈核上一级
 * @author eleven
 *
 */
public class QCXZJointSignFlowStageProcessor extends QCAbstractFlowStageProcessor {
	...
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + QCXZJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.XZJOINTSIGN_START;
	}
	...
	Map<String, PersonDetail> doNext(Flow flow, PersonDetail personDetail,
		...
		MyWork work = personDetail.buildWork(flow.getFlowType(),
				WorkStage.XZJOINTSIGN, prevWork);
		...
		if (prevWork != null) {
			...
			((XZJointSignWork) work).setWorknum(((XZJointSignWork) prevWork)
					.getWorknum());
			...
			if (((XZJointSignWork) prevWork).getParentId() != 0) {
				((XZJointSignWork) work).setParentId(((XZJointSignWork) prevWork)
						.getParentId());
			}

			// add by Cao_Shengyong 2014-03-25
			// 设置会办呈核上一级信息
			// 如果prevWork中Hb_ChengHe为2，使用了指派，那么肯定是呈核上一级后的主管进行的指派，这里需要将work的Hb_ChengHe设置为0
			//if (prevWork.getHb_ChengHe().equals("2")){
			//	((XZJointSignWork) work).setHb_ChengHe("0");
			//} else {
				((XZJointSignWork) work).setHb_ChengHe(((XZJointSignWork) prevWork)
						.getHb_ChengHe());
			//}
			((XZJointSignWork) work)
					.setHb_JoinSignStartId(((XZJointSignWork) prevWork)
							.getHb_JoinSignStartId());
			((XZJointSignWork) work)
					.setHb_JoinStartEmployeeId(((XZJointSignWork) prevWork)
							.getHb_JoinStartEmployeeId());
			((XZJointSignWork) work).setHb_ChengHeEnd(((XZJointSignWork) prevWork)
					.getHb_ChengHeEnd());
		}
		...
		//logger.info(flow.getFormNum() + " 对创建的work对象进行保存操作，这里是insert. " + work.getEmployeeId() + "(" + work.getFlowId() + "、" + work.getDeptId() + ")" + " Start...");
	...
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + QCXZJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.XZJOINTSIGN_START;
	}
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		logger.info("<=========================Begin.================================>");
		logger.info(flow.getFormNum() + " 职能部门会办完成操作.");
		Map<String, PersonDetail> result = new HashMap<String, PersonDetail>(2);
		// 完成myWork然后判断 本身是不是joinSignStartId，如果是的话，那么就结束分支会办的工作了
		if (XZJointSignWork.class.isInstance(work)) {
			...
			if (work.getHb_ChengHe().equals("1")
				...
				logger.info("参数值：" + work.getEmployeeId() + " # " + work.getPostCode());
				EmployeePos employeePos = personService.loadWideEmployeePos(work.getEmployeeId(), work.getPostCode());
				logger.info(flow.getFormNum() + " 根据 work 中的工号和岗位信息获取人员完整信息 End...");
				if (employeePos != null) {
					logger.info("返回值：" + employeePos.getEmployeeid() + " # " 
							+ employeePos.getMgDeptFlg() + " # " + employeePos.getMgA_DeptFlg() 
							+ " # " + employeePos.getMgCentFlg() + " $$ " 
							+ employeePos.getMgremployeeid() + " # " + employeePos.getMgrpostcode());
					logger.info("getHb_ChengHe: " + work.getHb_ChengHe());
