...
public class QCSystemJointSignFlowStageProcessor extends
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		...
		//logger.info(flow.getFormNum() + " 体系内会办　doComplete 操作 Start...");
		// 完成myWork然后判断 本身是不是joinSignStartId，如果是的话，那么就结束分支会办的工作了
		if (SystemJoinSignWork.class.isInstance(work)) {
		...
		//logger.info(flow.getFormNum() + " 体系内会办　doComplete 操作 End...");
		return result;
	}
	...
	Map<String, PersonDetail> doNext(Flow flow, PersonDetail personDetail,
		...
		if (prevWork != null) {
			...
			if (((SystemJoinSignWork) prevWork).getParentId() != 0) {
				((SystemJoinSignWork) work)
						.setParentId(((SystemJoinSignWork) prevWork)
								.getParentId());
			}
			...
			// 设置会办呈核上一级信息
			((SystemJoinSignWork) work)
					.setHb_ChengHe(((SystemJoinSignWork) prevWork)
							.getHb_ChengHe());
			((SystemJoinSignWork) work)
					.setHb_JoinSignStartId(((SystemJoinSignWork) prevWork)
							.getHb_JoinSignStartId());
			((SystemJoinSignWork) work)
					.setHb_JoinStartEmployeeId(((SystemJoinSignWork) prevWork)
							.getHb_JoinStartEmployeeId());
			((SystemJoinSignWork) work)
					.setHb_ChengHeEnd(((SystemJoinSignWork) prevWork)
							.getHb_ChengHeEnd());
		}
		...
		/*logger.info(flow.getFormNum() + " 体系内会办：对创建的work对象进行保存操作，这里是insert. " + work.getEmployeeId() + "("
				+ work.getFlowId() + "、" + work.getDeptId() + ")" + " Start...");*/
		flowDao.saveWork(work, flowService.getOrganDao());
		//logger.info(flow.getFormNum() + " 体系内会办：对创建的work对象进行保存操作，这里是insert. End...");
		//logger.info(flow.getFormNum() + " 体系内会办　doNext 操作 Start...");
		return result;
	}
	...
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + QCSystemJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.SYSTEMJOINTSIGN_START;
	}
	...
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + QCSystemJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.SYSTEMJOINTSIGN_START;
	}
