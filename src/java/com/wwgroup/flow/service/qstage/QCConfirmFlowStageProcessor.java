...
public class QCConfirmFlowStageProcessor extends QCAbstractFlowStageProcessor {
	...
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + QCConfirmFlowStageProcessor.class.getName());
	...
	Map<String, PersonDetail> doNext(Flow flow, PersonDetail personDetail,
		...
		//logger.info(flow.getFormNum() + " 本人确认　doNext 操作 Start...");
		// 获取申请人信息
		...
		/*logger.info(flow.getFormNum() + " 创建work对象并保存至数据库(insert.)"
				+ work.getEmployeeId() + "("
				+ work.getFlowId() + "、" + work.getDeptId() + ")" + "Start...");*/
		flowDao.saveWork(work, flowService.getOrganDao());
		//logger.info(flow.getFormNum() + " 创建work对象并保存至数据库. End...");
		result.put(actualPerson.getEmployeeId() + actualPerson.getPostCode(),
				actualPerson);
		logger.info(flow.getFormNum() + " 本人执行　doNext 操作 End...");
		return result;
	}

	@Override
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + QCConfirmFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.CONFIRM_START;
	}
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		Map<String, PersonDetail> result = new HashMap<String, PersonDetail>(2);
		//logger.info(flow.getFormNum() + " 本人确认　doComplete 操作 Start...");
		// 参数通过work得到
		//work.setStatus(FlowStatus.AGREE); //2016-09-30
		...
		//logger.info(flow.getFormNum() + " 本人确认　doComplete 操作 End...");
		return result;
	}
