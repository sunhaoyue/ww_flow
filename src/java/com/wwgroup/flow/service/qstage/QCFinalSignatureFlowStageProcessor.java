...
public class QCFinalSignatureFlowStageProcessor extends QCAbstractFlowStageProcessor {
	...
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + QCFinalSignatureFlowStageProcessor.class.getName());
	...
	Map<String, PersonDetail> doNext(Flow flow, PersonDetail personDetail, MyWork prevWork) {
		...
		if (prevWork != null && StringUtils.isNotEmpty(prevWork.getJoinSignStartId())) {
			...
			//logger.info(flow.getFormNum() + " 核决主管指派的流转过程,创建work对象并保存至数据库(insert.)" + work.getEmployeeId() + + work.getFlowId() + "、" + work.getDeptId() + ")" + "Start...");
		...
		MyWork work = mgrPerson.buildWork(flow.getFlowType(), WorkStage.BOSS_SIGN, null);
		work.setFlowId(flow.getId());
		...
		work.setEmployeenam(tmpPerson.getName());
		work.setTitlenam(tmpPerson.getTitname());
		//logger.info(flow.getFormNum() + " 核决主管的流转过程,创建work对象并保存至数据库(insert.)" + work.getEmployeeId() + "(" + work.getFlowId() + "、" + work.getDeptId() + ")" + "Start...");
		flowDao.saveWork(work, flowService.getOrganDao());
		//logger.info(flow.getFormNum() + " 核决主管的流转过程,创建work对象并保存至数据库. End...");
		result.put(mgrPerson.getEmployeeId() + mgrPerson.getPostCode(), mgrPerson);
		//logger.info(flow.getFormNum() + " 核决主管　doNext 操作 End...");
		return result;
	}

	@Override
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + QCFinalSignatureFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.FINAL_DECISION_START;
	}
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		Map<String, PersonDetail> result = new HashMap<String, PersonDetail>(2);
		//logger.info(flow.getFormNum() + " 核决主管　doComplete 操作 Start...");
		// 如果prevWork中的startEmployeeId不为空，代表是指派的流转过程
	   ...
	   //20170228
		...
		// 完成时判断是否还在会办，这里原则上剩余的都是总部行政职能部门
		...
		// 增加了董事长签核，这里就要去判断剩余的会办单位中是否还在职能部门和核决主管的选择
		...
		/**
		 * 如果会办单位中有职能部门，则先去会办职能部门
		 * 如果没有，但是勾选了董事长，则去董事长签核
		 * 否则直接去本人确认
		 */
		if (flow.getPendJointSignDeptIds() != null && flow.getPendJointSignDeptIds().length > 0){
			flow.setStatus(FlowStatus.XZJOINTSIGN_START);
	...
	private Map<String, PersonDetail> delegateJointSignWork(Flow flow, PersonDetail personDetail){
		...
		if (flow.getPendJointSignDeptIds() != null && flow.getPendJointSignDeptIds().length > 0){
			...
			for(i = 0; i < jointSIgnDeptIds.length; i++){
				...
				SystemGroups tmpGroups = flowService.loadGroupsById(Integer.valueOf(jointSignDeptId));
				if (tmpGroups.getSystemFlg().equals("Y")){
					// 如果是行政职能部门，则先去配置表中找到对应的人员
					/*HbMgrUser mgrUser = userService.getHbMgrUser(jointSignDeptId);
					PersonDetail person = new PersonDetail();
					if (mgrUser != null){
						person = personService.load
					}*/
					//System.out.println(" ############### ");
