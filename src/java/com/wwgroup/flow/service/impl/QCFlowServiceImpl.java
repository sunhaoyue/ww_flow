...
public class QCFlowServiceImpl implements QCFlowService {
	...
	public Map<String, PersonDetail> completeWork(MyWork work)
		...
		if (work.getStatus() != FlowStatus.DOING) {
			...
			MyWorkHistory tmp_work = flowDao.getHisWorkByID(work);
			String tmp_user = "";
			if (!StringUtils.isEmpty(tmp_work.getDlg_employeeid())){
				tmp_user = tmp_work.getDlgEmployeeNam();
			} else {
				tmp_user = tmp_work.getEmployeeNam();
			}
			throw new RuntimeException("该文件已由【" + tmp_user + "】执行提交/指派操作，请通过【已审核列表】或【高级查询】查看。");
		}
		// TODO: 使用event 替换if else 块，与上面的startNextWork一起重构
		//logger.info("执行　completeWork　前重新加载表单信息，确认flow为最新的　Start...");
		Flow flow = this.getFlow(work);
		//logger.info("执行　completeWork　前重新加载表单信息，确认flow为最新的　End...");
		// 判断
		//logger.info(flow.getFormNum() + " 执行　completeWork　Start...");
		Map<String, PersonDetail> map = flowStage.completeWork(flow, work);
		//logger.info(flow.getFormNum() + " 执行　completeWork　End...");
	...
	public Map<String, PersonDetail> assignWork(PersonDetail employee,
		...
		if (parentWork.getStatus() != FlowStatus.DOING){
			MyWorkHistory tmp_work = flowDao.getHisWorkByID(parentWork);
			String tmp_user = "";
			if (!StringUtils.isEmpty(tmp_work.getDlg_employeeid())){
				tmp_user = tmp_work.getDlgEmployeeNam();
			} else {
		...
		if (flow.getStatus() == FlowStatus.JOINTSIGN_START
				...
				|| flow.getStatus() == FlowStatus.CHENGHE_START
	...
	public void saveWork(MyWork work) {
		//work.setServerIP(CommonUtil.getServerIP());
		this.flowDao.saveWork(work, organDao);
	}

	public boolean hasJointSignFinished(Flow flow) {
		return this.flowDao.hasJointSignFinishedEx(flow);
	}
	...
	public MyWorkHistory[] listWorkHistory(String formNum) {
		...
		return flowDao.listWorkHistoryEx(formNum);
	}
	...
	public Map<String, PersonDetail> adminAssignWork(PersonDetail person,
		...
		PersonDetail tmpPerson = personService.loadWidePersonDetail(
				...
				myWork.getPostCode());
		myWork.setEmployeenam(tmpPerson.getName());
		myWork.setTitlenam(tmpPerson.getTitname());
	...
	public List<SystemGroups> getAllGroupsByParent(int parentID) {
		...
		return organDao.getAllGroupsByParent(groups.getOrgPath());*/
		return organDao.getAllGroupsByParent(String.valueOf(parentID));
	}

	public SystemGroups getRealCenterGroupsById(int groupId) {
		// 通过GROUPID获取SystemGroups信息
		SystemGroups groups = organDao.loadGroupsById(groupId);
		/**
		 * 由于总部组织存在四层架构的，所以要区别对待 总部只有父节点是705的才是中心<br/>
		 * GroupType: 3 单位 4 中心 5 部门
		 */
		SystemGroups result = groups;
		if (groups.getGroupType() == 3) {
			return groups;
		} else {
			if (groups.getCmpcod().equals("HQTR")) {
				/*
				 * int tmpParentId = groups.getParentID(); if (tmpParentId !=
				 * 705){ result = getRealCenterGroupsById(tmpParentId); } else {
				 * result = groups; }
				 */
				if (groups.getParentID() == 705) {
					return result;
				} else {
					while (groups.getParentID() != 705) {
						result = getRealCenterGroupsById(groups.getParentID());
						if (result.getParentID() == 705) {
							break;
						}
					}
				}
			} else {
				return organDao.getGroupsByDeptCode(groups.getDepcod(),
						groups.getA_depcod(), groups.getCmpcod(),
	...
	@Override
	public void updateFlowSubmitBoss(Flow flow){
		flowDao.updateFlowSubmitBoss(flow);
	}
	
	@Override
	public void updateFlowSubmitFBoss(Flow flow) {
		flowDao.updateFlowSubmitFBoss(flow);
	}
	
	@Override
	public void saveLog(LogDTO logDto){
		flowDao.saveLog(logDto);
	}
	
	@Override
	public boolean jointAgree(Flow flow, WorkStage workStage){
		boolean ret = true;
		List<MyWork> list = flowDao.getJointWorks(flow, workStage);
		if (list != null){
			for(int i = 0; i < list.size(); i++){
				MyWork work = list.get(i);
				if (work.getHb_Agree().equals("2")){
					ret = false;
					break;
				}
			}
