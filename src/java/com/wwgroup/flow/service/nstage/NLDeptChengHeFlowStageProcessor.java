...
public class NLDeptChengHeFlowStageProcessor extends
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		...
		//logger.info(flow.getFormNum() + " 部门主管 doComplete 代码片断 Start...");
		...
		//logger.info(flow.getFormNum() + " 部门主管 doComplete 代码片断 End...");
		return result;
	}

	@Override
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + NLDeptChengHeFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.DEPT_CHENGHE_START;
	}
	...
	Map<String, PersonDetail> doNext(Flow flow, PersonDetail personDetail,
			MyWork prevWork) {
		Map<String, PersonDetail> result = new HashMap<String, PersonDetail>(2);
		//logger.info(flow.getFormNum() + " 部门主管 doNext 代码片断 Start...");
		/**
		 * 需要判断传入用户（第一次传入的是申请人）是否是核决主管，
		 ...
		 * 没有会办，则直接去核决
		 */
		if (flow.getDecionmaker() == DescionMaker.DEPTLEADER) {
			...
			if (!StringUtils.isEmpty(flow.getJointSignDeptName())){
				if (employee.isDeptmgr()){
					...
					/*logger.info(flow.getFormNum() + " 部门主管节点继续汇报部门主管：对创建的work对象进行保存操作，这里是insert. "
							+ work.getEmployeeId() + "("
							+ work.getFlowId() + "、" + work.getDeptId() + ")" + " Start...");*/
					flowDao.saveWork(work, flowService.getOrganDao());
					//logger.info(flow.getFormNum() + " 部门主管节点继续汇报部门主管：对创建的work对象进行保存操作，这里是insert. End...");
		...
		//logger.info(flow.getFormNum() + " 部门主管 doNext 代码片断 End...");
		return result;
	}

	@Override
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + NLDeptChengHeFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.DEPT_CHENGHE_START;
	}
