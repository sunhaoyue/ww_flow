 ...
 * 
 */
public class NLInnerJointSignFlowStageProcessor extends
	...
	Map<String, PersonDetail> doComplete(Flow flow, MyWork work) {
		...
		int workCount = super.flowDao.getWorkCount(flow,
				WorkStage.INNERJOINTSIGN, FlowStatus.AGREE, flow.getFlowType());
		if (flow.getInnerJointSignIds().length == workCount) {
			...
			result.putAll(this.flowService.startNextWork(
					this.flowService.getFlow(work),
					this.flowService.getFlow(work).getActualPerson(), null));
		}
		return result;
	}
	...
	boolean completeValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " completeValidate: " + NLInnerJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.INNERJOINTSIGN_START;
	}
	...
	boolean startNextValidate(Flow flow) {
		//logger.info(flow.getFormNum() + " 状态：" + flow.getStatus() + " startNextValidate: " + NLInnerJointSignFlowStageProcessor.class.getName());
		return flow.getStatus() == FlowStatus.INNERJOINTSIGN_START;
	}
	...
	Map<? extends String, ? extends PersonDetail> doStart(Flow flow) {
		...
		if (this.flowService.saveFlow(flow)) {
			...
			for (int i = 0; i < personIds.length; i++) {
				...
				// TO DO
				PersonDetail personDetail = super.personService
						.loadWidePersonDetail(mixedValue[0], mixedValue[1],
								mixedValue[2]);
